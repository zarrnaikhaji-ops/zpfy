...
function defaultData() {
    ...
    if (!localStorage.getItem(STORAGE_KEYS.profile)) {
        localStorage.setItem(STORAGE_KEYS.profile, JSON.stringify({
            name: 'Zenz Official',
            bio: 'Penyedia file digital berkualitas. Hubungi kami untuk informasi lebih lanjut.',
            avatar: 'https://placehold.co/200x200/1a1e26/5a7a8a?text=Avatar'
        }));
    }
}
...
function renderProfile() {
    ...
    if (profile) {
        document.getElementById('profileName').textContent = profile.name || 'Nama Admin';
        document.getElementById('profileBio').textContent = profile.bio || 'Deskripsi tentang pemilik website...';
        const avatarImg = document.getElementById('profileAvatar');
        if (avatarImg) avatarImg.src = profile.avatar || 'https://placehold.co/200x200/1a1e26/5a7a8a?text=Avatar';
    }
}
...
function renderStore() {
    ...
    if (products.length) {
        productGrid.innerHTML = products.map(p => {
            ...
            const hasLink = p.fileLink && p.fileLink.trim() !== '';
            ...
            return `
                <div class="product-card">
                    ...
                    <div class="product-info">
                        ...
                        <div class="product-id">ID: ${escapeHtml(p.uniqueCode)}</div>
                        <div class="product-desc">${escapeHtml(p.description)}</div>
                        ${priceHtml}
                        <button class="btn-take ${!hasLink ? 'disabled' : ''}" data-id="${p.id}" data-name="${escapeHtml(p.name)}" ${!hasLink ? 'disabled' : ''}>
                            ${hasLink ? 'Ambil File' : 'Link belum tersedia'}
                        </button>
                    </div>
                </div>
            `;
...
function verifyAndDownload() {
    ...
    if (validCode) {
        if (product.fileLink && product.fileLink.trim() !== '') {
            window.open(product.fileLink, '_blank');
            closeCodeModal();
        } else {
            alert('Link file belum diatur oleh admin');
        }
    } else {
...
function loadAdminPanels() {
    ...
    const profile = JSON.parse(localStorage.getItem(STORAGE_KEYS.profile)) || { name: '', bio: '', avatar: '' };
            ...
            return `
                <div class="item-card">
                    <div style="flex:1;">
                        ...
                        <div class="file-link">Link: ${p.fileLink ? escapeHtml(p.fileLink.substring(0, 50)) + '...' : 'Belum diisi'}</div>
                        <div class="code-list">
                            <strong>Kode Aktif:</strong>
                            ${validCodes.map(c => `<span class="code-item">${c.code}</span>`).join('') || '<span class="code-expired">Tidak ada kode aktif</span>'}
                    ...
                    <div style="display: flex; flex-direction: column; gap: 0.3rem;">
                        ...
                        <button class="set-link" data-id="${p.id}">Set Link</button>
    ...
    document.getElementById('waNumberInput').value = wa;
    document.getElementById('adminSecretInput').value = adminSecret;
    document.getElementById('codeExpiryMinutes').value = expiryMinutes;
    document.getElementById('adminNameInput').value = profile.name || '';
    document.getElementById('adminBioInput').value = profile.bio || '';
    
    const adminAvatarPreview = document.getElementById('adminAvatarPreview');
    if (adminAvatarPreview) adminAvatarPreview.src = profile.avatar || 'https://placehold.co/200x200/1a1e26/5a7a8a?text=Avatar';
...
function attachAdminEvents() {
    ...
    document.querySelectorAll('.set-link').forEach(btn => {
        btn.onclick = () => {
            ...
            const idx = prods.findIndex(p => p.id == productId);
            if (idx !== -1) {
                const newLink = prompt('Masukkan link download (MediaFire/Google Drive):', prods[idx].fileLink || '');
                if (newLink !== null) {
                    prods[idx].fileLink = newLink;
                    localStorage.setItem(STORAGE_KEYS.products, JSON.stringify(prods));
                    loadAdminPanels();
                    renderStore();
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (addProductBtn) {
        addProductBtn.onclick = () => {
            ...
            let description = prompt('Deskripsi file');
            let price = prompt('Harga (Rp)');
            let discount = prompt('Diskon (%) - 0 jika tidak ada diskon', '0');
    ...
    if (saveProfileBtn) {
        saveProfileBtn.onclick = () => {
            ...
            const bio = document.getElementById('adminBioInput').value;
            const profile = JSON.parse(localStorage.getItem(STORAGE_KEYS.profile)) || {};
            profile.name = name;
            profile.bio = bio;
            ...
            if (avatarFile) {
                ...
                reader.onload = (e) => {
                    ...
                    localStorage.setItem(STORAGE_KEYS.profile, JSON.stringify(profile));
                    document.getElementById('adminAvatarPreview').src = profile.avatar;
                    renderProfile();
    ...
    if (submitCodeBtn) submitCodeBtn.onclick = verifyAndDownload;

    const closeCodeModalBtn = document.getElementById('closeCodeModal');
    if (closeCodeModalBtn) closeCodeModalBtn.onclick = closeCodeModal;
